...
@client.event
async def on_ready():
    await client.change_presence(status=discord.Status.dnd, activity=discord.Game("prefix is ~"))
    print("bot is ready")
...
@client.command()
async def logo(ctx):
  embed = discord.Embed(title="2d logo")
  ...
  embed = discord.Embed(title="3d logo")
